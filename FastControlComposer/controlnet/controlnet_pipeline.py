...
class FastComposerControlNetPipeline(
    DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin
):
    r"""
    Pipeline for text-to-image generation using FastComposer with ControlNet guidance.
    ...
    @torch.no_grad()
    def _encode_augmented_prompt(self, prompt: str, reference_images: List[PIL.Image.Image], device: torch.device, weight_dtype: torch.dtype):
        # TODO: check this 
    ...
    def __call__(
        ...
        alpha_: float = 0.0,
        reference_subject_images: List[PIL.Image.Image] = None,
        img_token: str = "img" 
    ):
        r"""
        ...
        assert do_classifier_free_guidance, "only support inference with cfg for now"
        assert guess_mode == False, "guess mode is not supported for now"
        ...
        # first compute text-only embeddings
        if isinstance(prompt, list):
            prompt_text_only = [p.replace(img_token, "") for p in prompt]
        else:
            prompt_text_only = prompt.replace(img_token, "")
        ...
        augmented_prompt_embeds = self._encode_augmented_prompt(prompt[0], reference_subject_images, device, prompt_embeds.dtype)
            ...
            for i, t in enumerate(timesteps):
                ...
                if i <= start_subject_conditioning_step:
                    ...
                    current_prompt_embeds = torch.cat(
                        [prompt_embeds.chunk(2)[0], augmented_prompt_embeds], dim=0
                    )
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=current_prompt_embeds, # prompt_embeds,
